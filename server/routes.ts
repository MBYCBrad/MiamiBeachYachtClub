...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/create-payment-intent", requireAuth, async (req, res) => {
    try {
      ...
      await auditService.logAction(req, 'create', 'payment_intent', paymentIntent.id, { amount, description });
      ...
      await auditService.logAction(req, 'create', 'payment_intent', undefined, req.body, false, error.message);
      res.status(500).json({ message: "Error creating payment intent: " + error.message });
    }
  });

  // Stripe webhook for payment confirmation (optional but recommended)
  app.post("/api/stripe-webhook", async (req, res) => {
    ...
    if (event.type === 'payment_intent.succeeded') {
      ...
      await auditService.logAction(
        { user: { id: parseInt(paymentIntent.metadata.userId || '0') } } as any,
        'payment_success',
        'payment_intent',
        paymentIntent.id,
