...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Get callable members for customer service dashboard
  app.get("/api/customer-service/members", requireAuth, requireRole([UserRole.ADMIN]), async (req, res) => {
    try {
      const users = await dbStorage.getAllUsers();
      const bookings = await dbStorage.getBookings();
      ...
      const callableMembers = users
        .filter(user => ['member', 'yacht_owner', 'service_provider'].includes(user.role))
        .map(user => {
          const currentBooking = activeBookings.find(b => b.userId === user.id);
          ...
          return {
            ...
            location: user.location || 'Miami',
            yachtName: currentBooking ? 'Marina Breeze' : undefined,
            currentTrip: !!currentBooking,
            bookingId: currentBooking?.id
          };
        });

      res.json(callableMembers);
    } catch (error: any) {
      console.error('Error fetching callable members:', error);
      res.status(500).json({ message: error.message });
    }
  });

  // Get customer service stats
  app.get("/api/customer-service/stats", requireAuth, requireRole([UserRole.ADMIN]), async (req, res) => {
    try {
      const users = await dbStorage.getAllUsers();
      const conversations = await dbStorage.getConversations();
      const bookings = await dbStorage.getBookings();

      const totalMembers = users.filter(u => ['member', 'yacht_owner', 'service_provider'].includes(u.role)).length;
      const activeConversations = conversations.filter(c => c.status === 'active').length;
      const onlineAgents = 3; // Static for now, could be dynamic with session tracking
      const callsInQueue = conversations.filter(c => c.priority === 'urgent' && c.status === 'active').length;
  ...
  app.post("/api/customer-service/call", requireAuth, requireRole([UserRole.ADMIN]), async (req, res) => {
    try {
      const { phoneNumber, memberName, callType = 'outbound' } = req.body;

      // Create Twilio call
      const call = await twilioClient.calls.create({
        to: phoneNumber,
        from: process.env.TWILIO_PHONE_NUMBER!,
        url: `${process.env.BASE_URL || 'https://your-domain.com'}/api/twilio/voice`,
        statusCallback: `${process.env.BASE_URL || 'https://your-domain.com'}/api/twilio/status`,
      ...
      await notificationService.createNotification({
        ...
        message: `Call initiated to ${memberName || phoneNumber}`,
        data: { callSid: call.sid, phoneNumber, memberName },
        priority: 'medium'
      });
  ...
  app.post("/api/twilio/voice", (req, res) => {
    ...
    twiml.say({
      voice: 'alice',
      language: 'en-US'
    }, 'Hello, this is Miami Beach Yacht Club customer service. Please hold while we connect you with an agent.');
    
    twiml.dial({
      callerId: process.env.TWILIO_PHONE_NUMBER
    }, '+1234567890'); // Replace with actual agent phone number
  ...
  // End call
  app.post("/api/customer-service/end-call", requireAuth, requireRole([UserRole.ADMIN]), async (req, res) => {
    try {
      ...
      await notificationService.createNotification({
        ...
        type: 'call_ended',
        title: 'Call Ended',
        message: `Call ${callSid} has been terminated`,
        data: { callSid },
        priority: 'low'
      });
